...
public class LongChipCompetition {
	...
	public void findTheLongestChip() {
		for (int i = 0; i < theBeatles.size(); i++) {
			for (int j = 0; j < theBeatles.get(i).getChips().size(); j++) {
				if (theBeatles.get(i).getChips().get(j).getLength() > largestChipNumber) {
					largestChipHolder = theBeatles.get(i).getName();
					largestChipNumber = theBeatles.get(i).getChips().get(j).getLength();
				}

			}
		}
System.out.println(largestChipHolder + " had the biggest chip with a length of " + largestChipNumber + " centimeters.");
	}
